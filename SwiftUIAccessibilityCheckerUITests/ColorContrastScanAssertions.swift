...
extension XCTestCase {
    ...
    /// Every row belonging to a control this screen tagged with `.srcLine()`, under ANY rule.
    ///
    /// The counterpart to `taggedContrastRows`, and the one to reach for when the question is
    /// whether the scan REACHED a control rather than what it concluded about its contrast.
    /// The two answer different questions and the difference matters: a control that produces
    /// no contrast row might have been skipped by the contrast rule or might not be on the
    /// screen at all, and only this one can tell those apart.
    func taggedRows(_ issues: [A11yIssue], sourceFile: String) -> [A11yIssue] {
        issues.filter { $0.element.contains("\(sourceFile):") }
    }

    /// The source lines named by the tagged rows, deduplicated and sorted.
    ///
    /// Several rules can report the same control, so the raw row count is a count of findings
    /// rather than of controls. The set of distinct lines is the control inventory, which is
    /// what a reachability assertion actually wants.
    func taggedSourceLines(_ issues: [A11yIssue], sourceFile: String) -> [Int] {
        let lines = taggedRows(issues, sourceFile: sourceFile).compactMap { row -> Int? in
            guard let r = row.element.range(of: "\(sourceFile):") else { return nil }
            let digits = row.element[r.upperBound...].prefix { $0.isNumber }
            return Int(digits)
        }
